            ...
            <div className="flex items-center gap-2 flex-wrap">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-[11px] uppercase tracking-[0.15em] font-semibold transition-all ${
                    cat === c ? "bg-olive text-cream" : "bg-cream text-ink hover:bg-wheat/60"
                  }`}
                >
                  {c}
                </button>
...
function ProductCard({
  ...
  return (
    <article className="group relative bg-white rounded-[28px] overflow-hidden border border-wheat/60 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_28px_60px_-24px_rgba(157,113,60,0.35)]">
      {product.badge && (
      ...
      <Link to="/product/$productId" params={{ productId: product.id }} className="block relative h-[260px] mx-4 mt-4 rounded-3xl bg-cream/50 overflow-hidden">
        <img
      ...
      <div className="px-6 pb-6 pt-5">
        <div className="flex items-center gap-1 text-gold text-xs">
          <Star size={12} fill="currentColor" /> <span>{product.rating.toFixed(1)}</span>
          <span className="text-ink/40">· {product.reviews} reviews</span>
        </div>
        <h3 className="mt-2 font-display text-2xl font-semibold text-olive">{product.name}</h3>
        <p className="mt-2 text-sm text-ink/70 line-clamp-2">{product.description}</p>
        <div className="mt-4 flex items-baseline justify-between">
          <p className="font-display text-2xl text-olive">₹{product.sizes[0].price}</p>
          <p className="text-xs text-ink/50">from {product.sizes[0].weight}</p>
        </div>
